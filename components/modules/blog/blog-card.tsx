...
const BlogCard: FC<{ article: ArticleIndex }> = ({ article }) => {
	return (
		<Link
			href={`/blog/${article.path.split("/").at(-1)}`}
		...
		>
			<Card
				className={
					"flex flex-col gap-4 h-full bg-secondary-background dark:bg-secondary-background-dark/70 backdrop-blur-3xl transition-all hover:brightness-90"
				}
				key={article.id}
			>
				{article.cover_image ? (
					...
					<div className="w-full h-56 rounded-lg bg-background/50 dark:bg-background-dark/50" />
				)}
